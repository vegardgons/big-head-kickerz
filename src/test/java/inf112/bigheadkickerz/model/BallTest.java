...
public class BallTest {
    ...
    void setUp() {
        ...
        try (MockedConstruction<Texture> mockedTexture = mockConstruction(Texture.class)) {
            ...
            ball = new Ball("BallImage.png", 5, 5);
    ...
    void testInitialPosition() {
        ...
        float expectedSize = ball.getSprite().getX() + ball.getSprite().getWidth() / 2; // Center of sprite
    ...
    void testApplyImpulse() {
        ...
        assertEquals(30, ball.getVelocityX(), 0.1f); // Impulse factor should be applied (x * 10)
        assertEquals(40, ball.getVelocityY(), 0.1f); // Impulse factor should be applied (y * 10)
    }

    @Test
    void testSetPosition() {
        ball.setPosition(10, 10);
        assertEquals(9.7, ball.getSprite().getX(), 0.1f); // Position should be set to 10, but sprite is centered
    ...
    void testReset() {
        ...
        assertEquals(4.7, ball.getSprite().getX(), 0.1f); // Initial position X, sprite is centered
    ...
    void testUpdate() {
        ...
        assertTrue(ball.getSprite().getX() > initialX); // Horizontal velocity should move the ball
        assertTrue(ball.getSprite().getY() > initialY); // Vertical velocity should move the ball
    }
    ...
    void testBounceOffWalls() {
        ...
        assertTrue(ball.getVelocityX() > 0); // Ball should bounce and change direction
    }
